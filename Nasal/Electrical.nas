...
var electrical_update = func {
  ...
  foreach(var supplier; suppliers) { # wieviel Strom kommt von den Suppliers?
    var volts = 0;
  ...
  foreach(var i; keys(components_lastvolts)) {					# Reset lastvolts for each component
    components_lastvolts[i] = -1;
										...
										# Propogate power:
  foreach(var connector; connectors) { # testen, ob alle Schalter an sind
...
var electrical_init = func {
  ...
  for(var i=0; i<size(connector_list); i+=1) {
    ...
    if (connector_list[i].getNode("scheme") == nil)
      { connector_list[i].getNode("scheme",1); connector_list[i].getNode("scheme").setValue(0); }
